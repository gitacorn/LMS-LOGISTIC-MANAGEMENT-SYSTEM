        ...
        <div class="container-fluid">
            ...
            <div class="row">
                <div class="col-lg-8 col-md-12">
                    <div class="lms-card">
                        <form method="POST" action="{{ url('announcement') }}" id="announcementForm">
                            @csrf
                            <div class="form-section-title">Announcement Details</div>
                                ...
                                <select id="announcement_template" class="form-control" style="border-radius: 8px;">
                                    <option value="" disabled selected>-- Select a Predefined Template --</option>
                                ...
                                <small class="text-muted">Choose a structured template or select Custom Message to write your own.</small>
                            </div>
                            <div class="form-group" id="announcement_text_wrapper" style="display: none;">
                                ...
                                <textarea name="v_announcement_text" id="v_announcement_text" class="form-control" rows="3" required maxlength="1000" placeholder="Type the scroll announcement text displayable in the marquee bar...">{{ old('v_announcement_text') }}</textarea>
                                <small class="text-muted">Maximum 1000 characters. You can edit the template text here.</small>
                        </div>

                            {{-- Hidden: dt_event_start_date is auto-set to event date --}}
                            <input type="hidden" name="dt_event_start_date" id="dt_event_start_date" value="{{ old('dt_event_start_date', date('Y-m-d')) }}">

                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                        <label for="dt_expiry_date" class="control-label">Event Date <span class="text-danger">*</span></label>
                                        <input type="date" name="dt_expiry_date" id="dt_expiry_date" class="form-control" value="{{ old('dt_expiry_date') }}" required min="{{ date('Y-m-d') }}">
                                    ...
                                    <select class="form-control select2" name="warehouse_ids[]" id="warehouse_ids" multiple required>
                                        ...
                                        @if(!empty($wareHouseDetails))
                                            @foreach($wareHouseDetails as $wareHouseDetail)
                                                @php $encodedId = trim(Wild_tiger::encode($wareHouseDetail->i_id)); @endphp
                        ...
                        <div class="row align-items-center mt-2 mb-3">
                            <div class="col-md-12">
                                    ...
                                    <div class="custom-control custom-switch d-flex align-items-center">
                                        <input type="checkbox" class="custom-control-input" id="email_enabled" name="email_enabled" value="1" {{ old('email_enabled') ? 'checked' : '' }}>
        ...
        <div class="modal-content" style="border-radius: var(--border-radius-lg);">
            ...
            <div class="modal-body">
                <div class="form-group">
                    ...
                    <input type="text" id="email_subject" name="email_subject" class="form-control custom-form-control" value="{{ old('email_subject', 'Warehouse Closure Update - [Eventdate] - [Day]') }}">
                ...
                <div class="form-group">
                    ...
                    <textarea id="email_message" name="email_message" class="form-control custom-form-control" rows="12">{{ old('email_message', 'Dear Team,
...
$(document).ready(function() {
    ...
    // Live Email Campaign schedule calculator
    function updateCampaignSchedule() {
        ...
        offsets.forEach(function(days) {
            ...
            var statusLabel = isPast ? 'Skipped (Date has passed)' : 'Pending (Scheduled)';
            var statusClass = isPast ? 'pill-skipped' : 'pill-pending';
    ...
    // Bind events for schedule updating (handled above in the sync block)
