...
<main class="page-height bg-light-color">
	<div class="breadcrumb-wrapper d-flex align-items-center border-navabr">
		...
		<?php if((checkPermission(config('permission_constants.ADD_GOODS_OUT_TO_AMAZON')) != false)){?>
			<a href="{{ config('constants.EUROPE_TO_AMAZON_MASTER_URL') . '/create' }}" class="btn btn btn-theme text-white button-actions-top-bar  border btn-sm mr-2 d-flex align-items-center" title="{{ trans('messages.add-to-amazon') }}"><i class=" fas fa-plus mr-md-1"></i> <span class="d-md-block d-none">{{ trans('messages.add') }}</span></a>
		<?php }?>
			...
			@if((checkPermission(config('permission_constants.EXCEL_EXPORT_GOODS_OUT_TO_AMAZON')) != false))
            	...
            	<button type="button" title="{{ trans('messages.export-summary') }}"  class="btn btn btn-theme text-white button-actions-top-bar  border btn-sm mr-2 d-flex align-items-center" onclick="exportData('{{ config('constants.ACTION_SUMMARY_EXPORT') }}');"><i class="fas fa-upload mr-md-1 fa-fw"></i><span class="d-md-block d-none">{{ trans("messages.export-summary") }}</span></button>
            @endif
		</div>
	</div>
	...
	<section class="inner-wrapper-common-section main-listing-section">
		<div class="container-fluid">
					...
					<div class="row align-items-center">
						<div class="col-xl-4 col-md-4">
							<div class="form-group">
								...
								<input type="text" class="form-control twt-enter-search custom-input" name="search_by_agent_warehouse" id="search_by_logistic_partner_name" placeholder="{{ trans('messages.search-by') }} {{ trans('messages.entry-no') }}, {{ trans('messages.tracking-no') }}, {{ trans('messages.workflow-id') }}, {{ trans('messages.shipment-id') }}">
							</div>
						</div>
						...
						<div class="col-xl-4 col-lg-3">
							<div class="form-group">
								<label class="control-label" for="search_account">{{ trans("messages.account") }}</label>
								<select name="search_account" class="form-control select2" onchange="filterData();">
									<option value="">{{ trans("messages.select") }}</option>
									<?php 
										if(!empty($comapanyMasterDetails)){
											foreach ($comapanyMasterDetails as $comapanyMasterDetail){
												$encodeId  = Wild_tiger::encode($comapanyMasterDetail->i_id);
												?>
												<option value="{{ $encodeId }}">{{ (!empty($comapanyMasterDetail->v_company_name) ? $comapanyMasterDetail->v_company_name : '' ) }}</option>
												<?php  
											}
										}
									?>
						...
						<div class="col-xl-3 col-lg-3 col-md-4 col-sm-6">
							<div class="form-group">
								<label class="control-label" for="search_to_location">{{ trans("messages.to-amazon-location") }}</label>
								<select name="search_to_location" class="form-control select2" onchange="filterData();">
									<option value="">{{ trans("messages.select") }}</option>
									<?php 
									if(!empty($locationDetails)){
										foreach ($locationDetails as $locationDetail){
											$encodeId  = Wild_tiger::encode($locationDetail->i_id);
											?>
                                        	<option value="{{ $encodeId }}">{{ (!empty($locationDetail->v_warehouse_name) ? $locationDetail->v_warehouse_name .(!empty($locationDetail->v_warehouse_code) ? ' (' .$locationDetail->v_warehouse_code .')'  : '' ) : '' )}}</option>
                                        	<?php 
                                    	}
                                    } 
									?>
						...
						<div class="col-xl-3 col-lg-3 col-md-4 col-sm-6">
							<div class="form-group">
								<label class="control-label" for="search_to_country_delivery">{{ trans("messages.to-country-delivery") }}</label>
								<select name="search_to_country_delivery" class="form-control select2" onchange="filterData();">
									<option value="">{{ trans("messages.select") }}</option>
									<?php 
			...
			<div class="filter-result-wrapper ajax-view">
				@include( config('constants.AJAX_VIEW_FOLDER') . 'europe-to-amazon/europe-to-amazon-list')
			</div>
		</div>
	</section>
 ...
 $(function(){
    ...
    $("[name='search_delivery_to_date']").datetimepicker().on('dp.change', function(e) {
	 ...
	 function searchField(){
	 	...
	 	var search_to_country_delivery = $.trim($('[name="search_to_country_delivery"]').val());
	 	...
	 	var searchData = {
		     ...
		     'search_to_country_delivery':search_to_country_delivery,
	 ...
	 function filterData(){
	 	...
	 	searchAjax(europe_to_amazon_module_url + 'filter' , searchFieldName);
	 }
	 var paginationUrl = europe_to_amazon_module_url + 'filter'
	 function exportData(type){
		var searchData = searchField();
		var export_info = {};
		export_info.url = europe_to_amazon_module_url + 'filter';
