...
<main class="page-height bg-light-color">
	<div class="breadcrumb-wrapper d-flex align-items-center border-navabr">
		...
		<?php if((checkPermission(config('permission_constants.ADD_SUPPLIER')) != false)){?>
			<a href="{{ config('constants.SUPPLIER_MASTER_URL') . '/create' }}" class="btn btn btn-theme text-white button-actions-top-bar  border btn-sm mr-2 d-flex align-items-center" title="{{ trans('messages.add-supplier') }}"><i class=" fas fa-plus mr-md-2"></i> <span class="d-md-block d-none">{{ trans('messages.add-supplier') }}</span></a>
		<?php }?>
					...
					<div class="row align-items-center">
						<div class="col-xl-5 col-md-12">
							<div class="form-group">
								...
								<input type="text" class="form-control twt-enter-search custom-input" name="search_by_supplier_name" placeholder="{{ trans('messages.search-by') }} {{ trans('messages.supplier-name') }}, {{ trans('messages.code') }}, {{ trans('messages.address')}}, {{ trans('messages.contact-person-name')}}, {{ trans('messages.contact-mobile')}} , {{ trans('messages.contact-email') }}">
							</div>
						</div>
						<div class="col-xl-3 col-md-4">
							<div class="form-group">
								<label class="control-label" for="search_supplier_country">{{ trans("messages.supplier-country") }}</label>
								<select class="form-control" name="search_supplier_country" onchange="filterData()">
						...
						<div class="col-xl-3 col-md-4">
							<div class="form-group">
								<label class="control-label" for="search_type">{{ trans("messages.type") }}</label>
								<select class="form-control" name="search_type" onchange="filterData()">
									<option value="">{{ trans("messages.select") }}</option>
									<?php 
										if(!empty($typeDetails)){
											foreach ($typeDetails as $key => $typeDetail){
												?>
												<option value="{{ $key }}">{{ (!empty($typeDetail) ? $typeDetail :'') }}</option>
						...
						<table class="table table-sm table-bordered table-hover" id="user-table">
							<thead>
								<tr>
									...
									<?php if( (checkPermission(config('permission_constants.EDIT_SUPPLIER')) != false) || (checkPermission(config('permission_constants.DELETE_SUPPLIER')) != false) ){?>
							...
							<tbody class="ajax-view">
                            	 @include( config('constants.AJAX_VIEW_FOLDER')  . 'supplier-master/supplier-master-list')
                            </tbody>
						</table>
					</div>
				</div>
...
function searchField(){
	var search_by_supplier_name = $.trim($('[name="search_by_supplier_name"]').val());
	...
	var searchData = {
            'search_by_supplier_name':search_by_supplier_name,
...
function filterData(){
	...
	searchAjax(supplier_module_url + 'filter' , searchFieldName);
}
var paginationUrl = supplier_module_url + 'filter'
</script>
