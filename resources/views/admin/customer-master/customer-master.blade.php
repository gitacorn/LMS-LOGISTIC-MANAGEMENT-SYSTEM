...
<main class="page-height bg-light-color">
	<div class="breadcrumb-wrapper d-flex align-items-center border-navabr">
		...
		<?php if((checkPermission(config('permission_constants.ADD_CUSTOMER')) != false)){?>
				<a href="{{ config('constants.CUSTOMER_MASTER_URL') . '/create' }}" class="btn btn btn-theme text-white button-actions-top-bar  border btn-sm mr-2 d-flex align-items-center" title="{{ trans('messages.add-customer') }}"><i class=" fas fa-plus mr-md-2"></i> <span class="d-md-block d-none">{{ trans('messages.add-customer') }}</span></a>
		<?php }?>
					...
					<div class="row align-items-center">
						<div class="col-xl-5 col-md-12">
							<div class="form-group">
								<label for="search_by_customer_name" class="control-label">{{ trans("messages.search-by") }}</label>
								<input type="text" class="form-control twt-enter-search custom-input" name="search_by_customer_name" placeholder="{{ trans('messages.search-by') }} {{ trans('messages.customer-name') }}, {{ trans('messages.code') }}, {{ trans('messages.address') }}, {{ trans('messages.contact-person-name') }}, {{ trans('messages.contact-mobile') }}, {{ trans('messages.contact-email')  }}">
							</div>
						</div>
						<div class="col-xl-3 col-md-4">
							<div class="form-group">
								<label class="control-label" for="search_customer_country">{{ trans("messages.customer-country") }}</label>
								<select class="form-control" name="search_customer_country" onchange="filterData()">
						...
						<table class="table table-sm table-bordered table-hover" id="user-table">
							<thead>
								<tr>
									...
									<th class="text-left">{{ trans("messages.customer-code") }}</th>
									<th class="text-left">{{ trans("messages.customer-address") }}</th>
									<th class="text-left">{{ trans("messages.customer-country") }}</th>
							...
							<tbody class="ajax-view">
                            	 @include( config('constants.AJAX_VIEW_FOLDER')  . 'customer-master/customer-master-list')
                            </tbody>
						</table>
					</div>
				</div>
...
function searchField(){
	var search_by_customer_name = $.trim($('[name="search_by_customer_name"]').val());
	var search_customer_country = $.trim($('[name="search_customer_country"]').val());
	var search_status = $.trim($('[name="search_status"]').val());
	var searchData = {
            'search_by_customer_name':search_by_customer_name,
            'search_customer_country': search_customer_country,
...
function filterData(){
	...
	searchAjax(customer_module_url + 'filter' , searchFieldName);
}
var paginationUrl = customer_module_url + 'filter'
</script>
