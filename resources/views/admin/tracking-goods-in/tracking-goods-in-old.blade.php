	...
	<div class="breadcrumb-wrapper d-flex align-items-center border-navabr">
		...
		<div class="ml-auto pt-sm-0 d-flex align-items-center">
			@if((checkPermission(config('permission_constants.EXCEL_EXPORT_TRACKING_GOODS_IN_REPORT')) != false))
            	<button type="button" title="{{ trans('messages.export-excel') }}"  class="btn btn btn-theme text-white button-actions-top-bar  border btn-sm mr-2 d-flex align-items-center" onclick="exportData();"><i class="fas fa-file-excel mr-1 fa-fw"></i><span class="d-md-block d-none">{{ trans("messages.export-excel") }}</span></button>
				...
				<div class="card card-body mb-3">
					<div class="row align-items-center">
						...
						<div class="col-xl-3 col-lg-3 col-md-4 col-sm-6">
							<div class="form-group">
								<label for="search_delivery_collection_location" class="control-label">{{ trans("messages.collection-delivery-location") }}</label>
						...
						<table class="table table-sm table-bordered table-hover" id="user-table">
							<thead>
								<tr>
									...
									<th class="text-left" style="max-width:90px;;min-width:90px">{{ trans("messages.buyer-name") }}</th>
									<th class="text-left" style="max-width:200px;;min-width:200px">{{ trans("messages.supplier-name") }} <br> {{ trans("messages.supplier-location") }}<br> {{ trans("messages.supplier-country") }}</th>
									...
									<th class="text-center">{{ trans("messages.logistic-delivery-type") }}</th>
									<th class="text-center" style="max-width:150px;;min-width:150px">{{ trans("messages.delivery-collection-location") }}</th>
...
$(document).ready(function() {
 ...
 $(function(){
    ...
    $("[name='search_delivery_to_date']").datetimepicker().on('dp.change', function(e) {
 	...
 	var tracking_goods_in_url_old = '{{config("constants.TRACKING_GOODS_IN_MASTER_URL_OLD")}}' + '/';
	 ...
	 function filterData(){
	 	...
	 	//searchAjax(tracking_goods_in_url_old + 'filter' , searchFieldName);
	 ...
	 var paginationUrl = tracking_goods_in_url_old + 'filter'
	 function exportData(type){
			var searchData = searchField();
			var export_info = {};
			export_info.url = tracking_goods_in_url_old + 'filter';
	  ...
	  function reintDataTable(className = null) {
	        ...
	        $('#' + className).DataTable({
	            ...
	            'columns': [{
	                ...
	                {
		                data:'logistic_delivery_type'
		            },
		            {
			            data:'delivery_collection_location'
		            },
		            {
	                    data: 'action',
	                    orderable: false
