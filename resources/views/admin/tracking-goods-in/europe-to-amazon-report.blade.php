...
<main class="page-height bg-light-color">
	<div class="breadcrumb-wrapper d-flex align-items-center border-navabr">
		...
		<div class="ml-auto pt-sm-0 d-flex align-items-center">
			@if(checkPermission(config('permission_constants.EXCEL_EXPORT_AMAZON_EU_FBA')) != false)
					...
					<div class="table-responsive fba-report-table fixed-tabel">
						<table class="table table-sm table-bordered table-hover" id="fba-table">
							...
							<tbody class="ajax-view">
								@include( config('constants.AJAX_VIEW_FOLDER') . 'tracking-goods-in/europe-to-amazon-report-list')
							</tbody>
						</table>
					</div>
				</div>
...
<script>
 	...
 	var fba_report_url = '{{config("constants.EUROPE_TO_AMAZON_REPORT_URL")}}' + '/';
	 ...
	 function filterData(){
	 	...
	 	searchAjax(fba_report_url + 'europeToAmazonFilter' , searchFieldName);
	 }
	 function exportData(){
		var searchData = searchField();
		var export_info = {};
		export_info.url = fba_report_url + 'europeToAmazonFilter';
